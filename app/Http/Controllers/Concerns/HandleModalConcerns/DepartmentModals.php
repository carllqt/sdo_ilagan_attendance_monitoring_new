...
trait DepartmentModals
{
    ...
    public function resolveAssignDivisionHeadModal(): ?array
    {
        if (! $this->isDepartmentModal('assign-division-head')) {
            return null;
        }

        $divisionName = trim((string) request('division_name', ''));

        if ($divisionName === '') {
            return null;
        }

        $division = Division::select('id', 'code', 'name')
            ->where('name', $divisionName)
            ->orWhere('code', $divisionName)
            ->first();
