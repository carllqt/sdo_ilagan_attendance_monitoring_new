...
class AttendanceMonitoringController extends Controller
{
    ...
    public function stream(AttendanceMonitoringRequest $request, AttendanceMonitoringEventStore $events)
    {
        return response()->stream(function () use ($request, $events) {
            @set_time_limit(0);
            ...
            $lastVersions = [
                'employee' => $events->version($stationId, 'employee')['version'] ?? null,
                'recent_logs' => $events->version($stationId, 'recent_logs')['version'] ?? null,
                'ranking' => $events->version($stationId, 'ranking')['version'] ?? null,
                'travel' => $events->version($stationId, 'travel')['version'] ?? null,
            ];
            $lastHeartbeat = time();
            ...
            while (! connection_aborted()) {
                ...
                $this->streamEmployeeUpdate($events, $stationId, $filter, $today, $lastVersions);
                $this->streamPanelUpdate(
                    $events,
                    $stationId,
                    'recent_logs',
                    'recent-logs-updated',
                    fn () => $this->recentLogs($today, $filter),
                    $lastVersions,
                );
                $this->streamPanelUpdate(
                    $events,
                    ...
                    'ranking',
                    'ranking-updated',
                    fn () => $this->topFirstTimeIns($today, $filter),
                    $lastVersions,
                );
                $this->streamPanelUpdate(
                    $events,
                    ...
                    'travel',
                    'travel-updated',
                    fn () => $this->travelOrders($today, $filter),
                    $lastVersions,
                );
    ...
    private function streamEmployeeUpdate(
        AttendanceMonitoringEventStore $events,
        int $stationId,
        AttendanceMonitoringFilter $filter,
        Carbon $today,
        array &$lastVersions,
    ): void {
        $version = $events->version($stationId, 'employee');
        $nextVersion = $version['version'] ?? null;

        if (! $nextVersion || $nextVersion === $lastVersions['employee']) {
            return;
        }

        $lastVersions['employee'] = $nextVersion;
        $employeeId = (int) ($version['employee_id'] ?? 0);

        if (! $employeeId) {
            return;
        }

        $this->sendSse('employee-updated', [
            'employee_id' => $employeeId,
            'employee' => $this->employeeRow($today, $filter, $employeeId),
        ]);
    }

    private function streamPanelUpdate(
        AttendanceMonitoringEventStore $events,
        int $stationId,
        string $type,
        string $event,
        ...
        $version = $events->version($stationId, $type);
        $nextVersion = $version['version'] ?? null;

        if (! $nextVersion || $nextVersion === $lastVersions[$type]) {
            return;
        }

        $lastVersions[$type] = $nextVersion;
        $this->sendSse($event, $payload());
    }

    private function sendSse(string $event, mixed $payload): void
    {
        echo "event: {$event}\n";
    ...
    private function employeeRow(Carbon $today, AttendanceMonitoringFilter $filter, int $employeeId): ?array
    {
        $todayAttendance = Attendance::query()
            ->selectRaw('employee_id, MAX(id) as attendance_id')
            ->whereDate('date', $today)
